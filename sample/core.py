...
def explicit_step(
    ...
    for i in prange(1, h):
        for j in range(1, w):
            ...
            dvx = ( a_u_x - bx[i, j] - damping_eff * vx[i, j]) #dvx = acc_x * dt
            dvy = ( a_u_y - by[i, j] - damping_eff * vy[i,j])
...
class ElasticProblem:
    ...
    def __init__(self,solid,elas_lambda,elas_mu,lm,
        ...
        if self.is_explicit:
            ...
            self.damping_eff = self.damping  # Effective damping for volumetric force computation
    ...
    def mod_solid(self,ix,iy,state, draw_fixed, size):
        ...
        return
    def mod_solid_update_solid(self):
    ...
    def explicit_step_numpy(self):
        ...
        acc_x = ( a_u_x - self.bx - self.damping_eff * self.vx) #division by vol_mass on next line for speed
        acc_y = ( a_u_y - self.by + self.damping_eff * self.vy)
